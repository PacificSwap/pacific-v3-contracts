...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of PACIFIC contract.
    IERC20 public immutable PACIFIC;
    ...
    uint256 public latestPeriodPacificPerSecond;
    ...
    /// @notice Record the pacific amount belong to MasterChefV3.
    uint256 public pacificAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IPacificV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    event NewUpkeepPeriod(
        ...
        uint256 startTime,
        uint256 endTime,
        uint256 pacificPerSecond,
        uint256 pacificAmount
    );
    event UpdateUpkeepPeriod(
        uint256 indexed periodNumber,
        uint256 oldEndTime,
        uint256 newEndTime,
        uint256 remainingPacific
    );
    ...
    /// @param _PACIFIC The PACIFIC token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _PACIFIC, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        PACIFIC = _PACIFIC;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the pacific per second , period end time.
    /// @param _pid The pool pid.
    /// @return pacificPerSecond Pacific reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 pacificPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            pacificPerSecond = (latestPeriodPacificPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the pacific per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return pacificPerSecond Pacific reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 pacificPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            pacificPerSecond =
                (latestPeriodPacificPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending PACIFIC rewards.
    /// @dev The pending pacific amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingPacific(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (PACIFIC.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, IPacificV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's PACIFIC allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotPacificNFT();
    ...
    /// @notice harvest pacific from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Pacific reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce pacificAmountBelongToMC.
        if (_token == address(PACIFIC)) {
            unchecked {
                // In fact balance should always be greater than or equal to pacificAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= pacificAmountBelongToMC) {
                    balance -= pacificAmountBelongToMC;
                } else {
                    // This should never happend.
                    pacificAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce pacificAmountBelongToMC.
        if (token == address(PACIFIC)) {
            unchecked {
                // In fact balance should always be greater than or equal to pacificAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= pacificAmountBelongToMC) {
                    balanceToken -= pacificAmountBelongToMC;
                } else {
                    // This should never happend.
                    pacificAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of pacific injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer pacific token from receiver.
        PACIFIC.safeTransferFrom(receiver, address(this), _amount);
        // Update pacificAmountBelongToMC
        unchecked {
            pacificAmountBelongToMC += _amount;
        }
        ...
        uint256 pacificPerSecond;
        uint256 pacificAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingPacific = ((latestPeriodEndTime - currentTime) * latestPeriodPacificPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingPacific);
            pacificAmount += remainingPacific;
        }
        pacificPerSecond = (pacificAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodPacificPerSecond = pacificPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, pacificPerSecond, pacificAmount);
    }

    /// @notice Update pacific reward for all the liquidity mining pool.
    ...
    /// @notice Update pacific reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer PACIFIC.
    /// @param _to The PACIFIC receiver address.
    /// @param _amount Transfer PACIFIC amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = PACIFIC.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update pacificAmountBelongToMC
            unchecked {
                if (pacificAmountBelongToMC >= _amount) {
                    pacificAmountBelongToMC -= _amount;
                } else {
                    pacificAmountBelongToMC = balance - _amount;
                }
            }
            PACIFIC.safeTransfer(_to, _amount);
        }
    }
