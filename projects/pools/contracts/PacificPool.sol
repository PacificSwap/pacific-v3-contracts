...
import "./interfaces/IVPacific.sol";

contract PacificPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 pacificAtLastUserAction; // keep track of pacific deposited at the last user action.
    ...
    event NewVPacificContract(address VPacific);
    ...
    /**
     * @notice Constructor
     * @param _token: Pacific token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: pacific pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(pacificPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the pacific owner address or the operator address.
     */
    modifier onlyOperatorOrPacificOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or pacific owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onPacificPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.pacificAtLastUserAction;
    ...
    /**
     * @notice Unlock user pacific funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrPacificOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Pacific Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in PACIFIC)
    ...
    function depositOperation(
        ...
        if (VPacific != address(0)) {
            IVPacific(VPacific).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.pacificAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Pacific Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VPacific != address(0)) {
            IVPacific(VPacific).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.pacificAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.pacificAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending PACIFIC tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingPacific = masterchefV2.pendingPacific(pacificPoolPID, address(this));
        if (pendingPacific > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(pacificPoolPID, 0);
    ...
    /**
     * @notice Set VPacific Contract address
     * @dev Callable by the contract admin.
     */
    function setVPacificContract(address _VPacific) external onlyAdmin {
        require(_VPacific != address(0), "Cannot be zero address");
        VPacific = _VPacific;
        emit NewVPacificContract(VPacific);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Pacific Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingPacificRewards();
     ...
     * @return Returns total pending pacific rewards
     */
    function calculateTotalPendingPacificRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingPacific(pacificPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingPacificRewards()) * (1e18)) / totalShares);
    }
