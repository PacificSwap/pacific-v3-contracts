 ...
 * Receiver will harvest pacific from MasterChefV2 pool , then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV1 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    /// @param _pacific Pacific address.
    /// @param _V2Pid Pid in MasterChefV2.
    constructor(IMasterChefV2 _V2, IMasterChefV3 _V3, IReceiver _receiver, IERC20 _pacific, uint256 _V2Pid) {
        MasterChefV2 = _V2;
        MasterChefV3 = _V3;
        Receiver = _receiver;
        Pacific = _pacific;
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            uint256 penddingPacficAmount = MasterChefV2.pendingPacific(V2Pid, address(Receiver));
            uint256 pacificBalanceInReceiver = Pacific.balanceOf(address(Receiver));
            uint256 latestPeriodEndTime = MasterChefV3.latestPeriodEndTime();
            if (penddingPacficAmount + pacificBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond)
                upkeepNeeded = true;
        }
    }
