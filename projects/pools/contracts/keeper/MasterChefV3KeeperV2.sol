 ...
 * Receiver will receive pacific token, then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV2 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    constructor(IMasterChefV3 _V3, IReceiver _receiver, IERC20 _pacific) {
        MasterChefV3 = _V3;
        Receiver = _receiver;
        Pacific = _pacific;
    }
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            ...
            if (pacificBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond) upkeepNeeded = true;
        }
    }
