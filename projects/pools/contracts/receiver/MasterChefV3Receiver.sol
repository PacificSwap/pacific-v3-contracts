...
contract MasterChefV3Receiver is Ownable {
    ...
    /// @param _pacific Pacific token address.
    /// @param _v2Pid The pool id of the dummy pool on the MCV2.
    constructor(IMasterChefV2 _v2, IMasterChefV3 _v3, IERC20 _pacific, uint256 _v2Pid) {
        MasterChefV2 = _v2;
        MasterChefV3 = _v3;
        Pacific = _pacific;
        V2_Pid = _v2Pid;

        Pacific.safeApprove(address(_v3), type(uint256).max);
    }
    ...
    /// @notice Harvest pending PACIFIC tokens from MasterChef V2.
    ...
    /// @notice Withdraw unexpected tokens sent to the receiver, can also withdraw pacific.
    /// @dev Callable by owner.
