...
import '@pacificswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@pacificswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@pacificswap/v3-periphery/contracts/libraries/Path.sol';
import '@pacificswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@pacificswap/v3-core/contracts/interfaces/IPacificV3Pool.sol';
import '@pacificswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IPacificV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IPacificV3Pool pool) {
        pool = IPacificV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IPacificV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IPacificV3Pool(pool).slot0();
            }
