...
/// @title Canonical Pacific V3 factory
/// @notice Deploys Pacific V3 pools and manages ownership and control over pool protocol fees
contract PacificV3Factory is IPacificV3Factory {
    /***/
    ...
    function createPool(
        ...
        pool = IPacificV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IPacificV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IPacificV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IPacificV3Pool(pool).setLmPool(lmPool);
    }
}