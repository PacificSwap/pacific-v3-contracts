...
contract TestPacificV3Router is IPacificV3SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IPacificV3Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function pacificV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IPacificV3Pool(msg.sender).token0()
                : IPacificV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IPacificV3Pool(pools[0]).token1();
            IPacificV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IPacificV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IPacificV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
