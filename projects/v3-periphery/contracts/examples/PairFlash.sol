...
import '@pacificswap/v3-core/contracts/interfaces/callback/IPacificV3FlashCallback.sol';
...
/// @notice An example contract using the PacificV3 flash function
contract PairFlash is IPacificV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `PacificV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IPacificV3Pool pool = IPacificV3Pool(PoolAddress.computeAddress(deployer, poolKey));
